...
def qichexinxi_page(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        try:
            __hasMessage__=qichexinxi.__hasMessage__
        except:
            __hasMessage__=None
        if  __hasMessage__=="是":
            tablename=request.session.get("tablename")
        ...
        if  __isAdmin__ == "是":
            ...
            if tablename!="users" and 'qichexinxi'[:7]!='discuss'and "userid" in qichexinxi.getallcolumn(qichexinxi,qichexinxi):
                req_dict["userid"] = request.session.get("params").get("id")
...
def qichexinxi_update(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        if req_dict.get("mima") and req_dict.get("password"):
            if "mima" not  in qichexinxi.getallcolumn(qichexinxi,qichexinxi) :
                del req_dict["mima"]
            if  "password" not  in qichexinxi.getallcolumn(qichexinxi,qichexinxi) :
...
def qichexinxi_sendemail(request):
    if request.method in ["POST", "GET"]:
        req_dict = request.session.get("req_dict")
...
def qichexinxi_autoSort2(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        leixing = set()
        try:
            cursor.execute("select inteltype from storeup where userid = %d"%(request.session.get("params").get("id"))+" and tablename = 'qichexinxi' order by addtime desc")
        ...
        L = []
        cursor.execute("select * from qichexinxi where $intelRecomColumn in ('%s"%("','").join(leixing)+"') union all select * from qichexinxi where $intelRecomColumn not in('%s"%("','").join(leixing)+"')")
...
def qichexinxi_value(request, xColumnName, yColumnName, timeStatType):
    if request.method in ["POST", "GET"]:
        ...
        sql = ''
        if timeStatType == '日':
            sql = "SELECT DATE_FORMAT({0}, '%Y-%m-%d') {0}, sum({1}) total FROM qichexinxi {2} GROUP BY DATE_FORMAT({0}, '%Y-%m-%d')".format(xColumnName, yColumnName, where, '%Y-%m-%d')

        if timeStatType == '月':
            sql = "SELECT DATE_FORMAT({0}, '%Y-%m') {0}, sum({1}) total FROM qichexinxi {2} GROUP BY DATE_FORMAT({0}, '%Y-%m')".format(xColumnName, yColumnName, where, '%Y-%m')

        if timeStatType == '年':
            sql = "SELECT DATE_FORMAT({0}, '%Y') {0}, sum({1}) total FROM qichexinxi {2} GROUP BY DATE_FORMAT({0}, '%Y')".format(xColumnName, yColumnName, where, '%Y')
        L = []
        cursor = connection.cursor()
        cursor.execute(sql)
        desc = cursor.description
...
def qichexinxi_o_value(request, xColumnName, yColumnName):
    if request.method in ["POST", "GET"]:
        ...
        sql = "SELECT {0}, sum({1}) AS total FROM qichexinxi {2} GROUP BY {0}".format(xColumnName, yColumnName, where)
        L = []
        cursor = connection.cursor()
        cursor.execute(sql)
        desc = cursor.description
...
def qichexinxi_alipay(request):
    if request.method in ["POST", "GET"]:
        ...
        order_string = alipay.api_alipay_trade_page_pay(
            ...
            return_url='http://localhost:8080/djangozq00i/qichexinxi/notify',
            #notify_url=''
        )
...
def qichexinxi_notify(request):
    if request.method in ["POST", "GET"]:
        req_dict = request.session.get("req_dict")
        out_trade_no = req_dict['out_trade_no']
        cursor = connection.cursor()
        
        return redirect('http://localhost:8080/djangozq00i/admin/dist/index.html#/qichexinxi')
